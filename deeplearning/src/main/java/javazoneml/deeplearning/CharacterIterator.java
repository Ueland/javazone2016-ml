...
/**
 * https://github.com/deeplearning4j/dl4j-examples/blob/master/dl4j-examples/src/main/java/org/deeplearning4j/examples/recurrent/character/CharacterIterator.java
 */
public class CharacterIterator implements DataSetIterator {
     ...
     */
    public CharacterIterator(List<String> lines, Charset textFileEncoding, int miniBatchSize, int exampleLength,
        ...
        if (currIdx == characters.length) {
            ...
            textCharacters = Arrays.copyOfRange(characters, 0, currIdx);
        }
        if (exampleLength >= textCharacters.length) throw new IllegalArgumentException("exampleLength=" + exampleLength
                + " cannot exceed number of valid characters in file (" + textCharacters.length + ")");

        int nRemoved = maxSize - textCharacters.length;
        System.out.println("Loaded and converted file: " + textCharacters.length + " valid characters of "
    ...
    public char convertIndexToCharacter(Integer idx) {
        return validCharacters[idx];
    }

    public int convertCharacterToIndex(Character c) {
    ...
    public Map<Character, Integer> getCharToInt(){
        return charToIdxMap;
    }
    ...
    public DataSet next(int num) {
        ...
        for (int i = 0; i < currMinibatchSize; i++) {
            ...
            int currCharIdx = charToIdxMap.get(textCharacters[startIdx]);    //Current input
            int c = 0;
            for (int j = startIdx + 1; j < endIdx; j++, c++) {
                int nextCharIdx = charToIdxMap.get(textCharacters[j]);//Next character to predict
    ...
    public int totalExamples() {
        return (textCharacters.length - 1) / miniBatchSize - 2;
    }
    ...
    public void reset() {
        ...
        int nMinibatchesPerEpoch = (textCharacters.length - 1) / exampleLength - 2;   //-2: for end index, and for partial example
